...
String _charEnc;
	 ...
	 *            main applet.
	 */
	public EvaluateThread(String servername, int portNum, IRecorder rec,
			String mode, String charEnc) {
	...
	public void run() {
		// open a connection
		openConnection(Constants.MODE_EVALUATE);
		// read feedback from server (emoresult).
		stringReader = new BufferedReader(new InputStreamReader(in));
		String receivedString = "", evalResultString = "";
		try {
			out.writeBytes(_mode + '\n');
			while (!(receivedString = stringReader.readLine())
					.equals("finished")) {
				evalResultString += URLDecoder.decode(receivedString,_charEnc) + "\n";
			}
		} catch (Exception e) {
		...
		// close
		closeConnection();
//		System.err.println(evalResultString);
		rec.setMessage(evalResultString);
	}
